...
if __name__ == "__main__":
    ...
    # data = mani_dataset(r'CASIA2.0_revised', edge_width=5, transform=transform, if_return_shape=True)
    ...
    # Example with mani_dataset (can also use json_dataset)
    dataset = mani_dataset('CASIA2.0_revised')

    for idx in range(len(dataset)):
        tp_path = dataset.tp_path[idx]
        gt_path = dataset.gt_path[idx]
        print(f"{idx}: TP -> {tp_path} | GT -> {gt_path}")

    
    # path = r"CASIA2.0_revised"          # GPU-server dir
